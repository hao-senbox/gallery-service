...
const (
	...
	ttl         = time.Second * 15
	checkId     = "gallery-service-health-check"
)
...
func (c *service) updateHealthCheck() {
	...
	for {
		...
		//_, _, err := c.client.Agent().AgentHealthServiceByID(serviceId)
		if err != nil {
...
func (c *service) setupConsul() {
	...
	//if c.cfg.Consul.Host != "localhost" {
	//	// Fallback to localhost if the host is not set in the config
	...
	check := &api.AgentServiceCheck{
		...
		//Interval: "15s",                                                     // Interval for health check
		//Timeout:  "30s",
	...
	registration := &api.AgentServiceRegistration{
		...
		Tags:    []string{"go", "gallery-service"},
		Check:   check,
	}
